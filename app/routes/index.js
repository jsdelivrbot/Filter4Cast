...
//Number of days
const LONG_DAYS_PERIOD = 30;
const SHORT_DAYS_PERIOD=10;


//----------------------//

var resetDate_l_asDate_co2 = 1499903882588 + 86400000*20
var resetDate_l_asDate_co1 = 1499903882588 + 10* 86400000
...
router.post('/getTopTagspost',function(req,res){
    ...
    uri: process.env.timeseriesURL || 'https://time-series-store-predix.run.aws-usw02-pr.ice.predix.io/v1/datapoints',
        body: rawBody,
        method: 'POST',
        json: true
}).then(data => {
            console.log(data.tags[0].name);
    var date_voc_long = (Date.now() - LONG_DAYS_PERIOD * 86400000); 
            
    var date_voc_short = (Date.now() - SHORT_DAYS_PERIOD * 86400000);
            
            var date_reset = "";
            if(data.tags[0].name == "unternehmer-3dlaser-co1")  date_reset = resetDate_l_asDate_co1;
            else if (data.tags[0].name == "unternehmer-3dlaser-co2") date_reset = resetDate_l_asDate_co2;
            else console.log("YOLO - wrong name");
                
    var dateNewFormat = dateformat(date_reset, "dd-mm-yyyy HH:MM:ss")        
    ...
    var limit = 0;
    limit = data.tags[0].results[0].values.length // all of the dataset from first day of measurement
    ...
    for (i = 0; i < limit; i++) { 
        ...
        if (changeRequired == true) {
            nextDay = (data.tags[0].results[0].values[i][0] + 86400000) ;
        ...
        currentTime = data.tags[0].results[0].values[i][0];
        ...
        if (currentTime < nextDay && currentTime> date_voc_long) {
            sum_voc_long_per_day += parseInt(data.tags[0].results[0].values[i][1]);
        }
        if (currentTime < nextDay && currentTime > date_voc_short) {
            sum_voc_short_per_day += parseInt(data.tags[0].results[0].values[i][1]);
        }

        if (currentTime < nextDay && currentTime > date_reset) {
            sum_voc_current_per_day += parseInt(data.tags[0].results[0].values[i][1]);
            totalSumCurrentPlot+=parseInt(data.tags[0].results[0].values[i][1]);
            plotValues.push(totalSumCurrentPlot);
            plotTimes.push(currentTime);
            var val = [currentTime, totalSumCurrentPlot ];
    ...
    var a= currentTime + prediction * 86400000;
    var val = [a, max_voc];
            console.log(data.tags[0].results[0].values[4][0]);
            
    datatemp.push(val);
    
    var YOLO = {
        "prediction": prediction,
        "filterName": data.tags[0].name,
...
router.post('/getValuesFile',function(req,res) {
    ...
    if(req.body.feed_key == "co1"){
        ...
        var resetDate_l = (new Date(resetDate_l_asDate_co1)).getTime() / 1000
         filterName = "Filter Lasercutter 2 (Sensor CO1)"
        }
        ...
        var resetDate_l = (new Date(resetDate_l_asDate_co2)).getTime() / 1000
         filterName = "Filter Lasercutter 1 (Sensor CO2)"
        }
